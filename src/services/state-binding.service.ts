...
export interface AdapterStateApi {
  ...
  getForeignObjectsAsync(
    pattern: string,
    type: "state" | "enum",
  ): Promise<Record<string, IoBrokerObject | null | undefined>>;
  getForeignStatesAsync(
    pattern: string | string[],
  ): Promise<Record<string, IoBrokerState | null | undefined>>;
...
export class StateBindingService {
  ...
  async searchObjects(query = "", limit = 80): Promise<StateOption[]> {
    ...
    const results = Object.entries(objects)
      .filter((entry): entry is [string, IoBrokerObject] => Boolean(entry[1]))
      .map(([id, object]) => {
...
function localizedNames(name: string | Record<string, string> | undefined): string[] {
  if (!name) {
    return [];
  }
  ...
  return [...new Set([name.en, name.de, ...Object.values(name)].filter(Boolean))] as string[];
}

function matchesQuery(option: StateOption, query: string): boolean {
  if (!query) {
    return true;
  }
  return [
...
function attachState(option: StateOption, state: IoBrokerState | null | undefined): void {
...
function findEnumNames(
  id: string,
  enums: Record<string, IoBrokerObject | null | undefined>,
): string[] {
  const names: string[] = [];
  for (const object of Object.values(enums)) {
    ...
    const belongs = object.common?.members?.some(
      (member) => id === member || id.startsWith(`${member}.`),
    );
    if (belongs) {
