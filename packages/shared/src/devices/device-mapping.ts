...
export type DeviceKind =
  "light" | "thermostat" | "blind" | "sensor" | "scene" | "energy" | "camera";
...
const componentKinds: Partial<Record<ComponentType, DeviceKind>> = {
  "light-card": "light",
...
const rules: Record<DeviceKind, TargetRule[]> = {
  light: [
...
export function detectDeviceMapping(
  ...
  const bindings: DeviceMappingBinding[] = [];
  ...
  const bestScore = Math.max(
    ...targetRules.map((rule) => scoreTarget(selected, rule, selected, 0)),
  );
  return {
    kind,
...
function adaptRulesForComponent(
  source: TargetRule[],
...
function detectKind(group: StateOption[], selected: StateOption): DeviceKind {
  const scored = (Object.keys(rules) as DeviceKind[]).map((kind) => ({
    kind,
    score: Math.max(
      ...group.flatMap((candidate) =>
        rules[kind].map((rule, index) => scoreTarget(candidate, rule, selected, index)),
      ),
    ),
  }));
...
function bindingMode(candidate: StateOption, rule: TargetRule): BindingMode {
  if (rule.writable) {
...
function primaryTarget(componentType: ComponentType | undefined): string {
