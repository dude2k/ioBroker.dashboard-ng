...
- Current adapter version: `0.7.0`
- Current GitHub tag: `v0.7.0`
...
- Nested sections and containers with hierarchy-aware editing.
- Multi-selection alignment and distribution tools.
...
- Added first-class Section and Container components with nested twelve-column
  child grids in Editor and Viewer.
- Added safe hierarchy validation, recursive copy, duplicate, template and
  delete behavior, plus an additive schema v2-to-v3 migration.
- Added lightweight edge, center and distribution tools for multi-selection.
- Added central theme controls, four curated presets and hardened kiosk,
  fullscreen, Wake Lock and burn-in behavior.
...
- Added the AP4 inspector foundation for MVP card properties, target-specific
  state bindings, actions, visibility and conditional styles.
- Prepared formula bindings in the inspector so component targets can calculate
  values from a selected input state through the safe formula parser.
- Kept Editor and Viewer on the shared runtime path for visibility,
  conditional styles and formula target state resolution.

### 0.3.16 (2026-07-03)

- Left-aligned the Viewer grid so saved dashboard layouts start at the left
  page edge instead of being horizontally centered in wide browser windows.
...
- Made the Viewer load dashboards through the adapter `dashboard.load` command
  before trying the web adapter file URL.
- Kept the web file URL as a fallback only, avoiding stale/default Viewer data
  when the file endpoint lags behind adapter storage.
- Added Viewer-side load diagnostics in the browser console for `sendTo`, file
  fetch and fallback paths.

### 0.3.13 (2026-07-03)

- Made Editor saves use the adapter `dashboard.save` command first instead of
  the browser-side file write path that could corrupt the saved JSON.
- Made save verification fatal so the Editor no longer reports `Saved` when the
  written dashboard cannot be read back as the same JSON.
- Added recovery for newer local Editor dashboards when adapter storage still
  contains an older or broken file.
- Added backend save read-back verification and kept the adapter running if the
  startup dashboard load fails, so a later Editor save can repair storage.

### 0.3.12 (2026-07-03)

- Fixed Editor/Admin socket creation by using `@iobroker/socket-client`
  `AdminConnection` instead of accepting a raw Socket.IO `emit`-only socket.
- Ignored emit-only sockets without ioBroker helper methods so load/save no
  longer gets stuck on unanswered raw socket events.
- Forced the admin socket path to `/socket.io` for adapter admin pages opened
  under `/adapter/dashboard-ng/index_m.html`.

### 0.3.11 (2026-07-03)

- Added trace-based debug logging across Editor save/load, browser socket file
  APIs, `sendTo` fallback and adapter storage operations.
- Added save verification diagnostics after direct file writes so mismatches
  between Editor and Viewer storage can be identified.
- Added `info.lastDebugLog` as a readonly adapter state with the latest backend
  debug line.

### 0.3.10 (2026-07-02)

- Added an in-editor diagnostics panel for dashboard load/save debugging.
- Added copyable diagnostics with URL, browser user agent, current editor
  status and the last file/socket/sendTo operations.
- Made frontend diagnostics safe so logging cannot break save/load paths in
  minimal ioBroker or test environments.
...
- Device detection is heuristic and needs broader testing across real-world
  adapter ecosystems.
- Real ioBroker installation coverage still needs expansion before `1.0.0`.
